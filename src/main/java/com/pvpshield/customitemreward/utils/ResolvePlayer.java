...
public class ResolvePlayer {
    ...
    public final static Player getPlayerFromPlayerRef(PlayerRef playerRef) {
        Ref<EntityStore> ref = Universe.get().getPlayer(playerRef.getUuid()).getReference();
        Store<EntityStore> store = Universe.get().getPlayer(playerRef.getUuid()).getReference().getStore();

        return ResolvePlayer.getPlayer(ref, store);
    }
    ...
    public static Store<EntityStore> getStoreByEvent(PlayerReadyEvent event) {
        Player player = ResolvePlayer.getPlayerFromEvent(event);
        Ref<EntityStore> ref = player.getReference();
        return ref.getStore();
    }
    @Nonnull
